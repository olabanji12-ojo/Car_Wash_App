...
func (rs *ReviewService) CreateReview(userID string, input models.Review) (*models.Review, error) {
	...
	// 2. Check if order_id is provided and validate
...
// ReplyToReview allows a business owner to reply to a review
func (rs *ReviewService) ReplyToReview(reviewID string, response string) error {
	...
	return rs.reviewRepository.AddResponse(reviewObjID, response)
}

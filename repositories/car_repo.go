...
func (cr *CarRepository) GetCarByID(carID primitive.ObjectID) (*models.Car, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
...
func (cr *CarRepository) UpdateCar(carID primitive.ObjectID, updates bson.M) error {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
...
func (cr *CarRepository) DeleteCar(carID primitive.ObjectID) error {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
...
func (cr *CarRepository) GetCarsByUserID(userID primitive.ObjectID) ([]models.Car, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
...
func (cr *CarRepository) SetDefaultCar(userID, carID primitive.ObjectID) error {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()

	// Step 1: Unset all other defaults
	err := cr.UnsetDefaultCarsForUser(userID)
	if err != nil {
		return err
	}
...
func (cr *CarRepository) UnsetDefaultCarsForUser(userID primitive.ObjectID) error {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
